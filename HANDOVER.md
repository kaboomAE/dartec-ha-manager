...
**Current version**: 0.23.0 in `manifest.json`, not yet released (0.22.0, released 2026-09-19, is the latest release)
...
**Room panels (0.23.0).** Four commands, contract in the planner's Phase 3 contract (room dashboards and room panels, 2026-09-19); rules in `panels.py`:
...
| 0.23.0 | **Room panels** (Phase 3 of the planner-manager integration). `panel_setup` (sensitive), `panel_update`, `panel_remove`, `panel_status` (routine) for `panel-<slug>` accounts: non-admin, local-only, first dashboard = the room's dashboard, every other sidebar entry hidden (`panels.py`, `panel_cmds.py`, §6). Snapshot gains `panels` and `core.language`. My Home: kind `panel`, not a member, not counted, not changeable, one read-only line, `panel-` reserved; `dartec-room-*` dashboards never offered. Arabic strings added unreviewed, like the rest of `ar.json`. New CI job `live-panels`. Not released |
